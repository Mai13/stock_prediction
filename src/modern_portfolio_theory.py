# https://www.kaggle.com/sestostudio/modern-portfolio-theory
...
price_df = pd.read_csv(f'{filepath.resolve().parent}/stock_market_historical_data/prices.csv')
fund_df = pd.read_csv(f'{filepath.resolve().parent}/stock_market_historical_data/fundamentals.csv')
sec_df = pd.read_csv(f'{filepath.resolve().parent}/stock_market_historical_data/securities.csv')
...
def get_stock(sector):
    ...
    performance = price_df.groupby('symbol')['return'].apply(lambda x: (gmean(x) - 1) * 252).sort_values(
        ascending=False)
